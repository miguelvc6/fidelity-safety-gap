...
- Build the passive suite with `--use-unlabeled-interim`. Its payload must be
  byte-identical to the archived pre-rewrite generation; the hierarchy appears
  only in provenance and never as neural input nodes.
